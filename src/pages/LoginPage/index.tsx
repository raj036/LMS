...
export default function LoginPage() {
  ...
  const handleUpdate = async (e: any) => {
    e.preventDefault();
    try {
      const email = encodeURIComponent(forgetData.email);
      const newPassword = encodeURIComponent(forgetData.new_password);
      const confirmNewPassword = encodeURIComponent(
        forgetData.confirm_new_password
      );

      const url = `/api/reset_password?email=${email}&new_password=${newPassword}&confirm_new_password=${confirmNewPassword}`;
        ...
        <div className="flex flex-row sm:flex-col-reverse justify-between items-start w-full my-[75px] sm:my-1 mx-auto max-w-[1350px]">
          <div className="flex flex-col items-center justify-start w-[70%] sm:w-[100%] md:w-[140%] p-5 border rounded-lg border-teal-900 bg-teal-900/5">
            ...
            {error && (
             ...
             {forgetError && <p className="text-[red] mt-4">Please enter same password</p>}
          </div>
